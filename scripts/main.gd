...
func _build_cavern() -> void:
	...
	_place_gems(12 + floor_index * 2 + _effective_extra_gems_bonus())
	_place_super_gems(SUPER_GEM_BASE_COUNT + 1 + mini(_effective_super_gem_bonus(), 2) + (1 if floor_index >= 5 else 0))
...
func _inflate_lance_target(enemy_i: int, amount: int) -> bool:
	...
	enemy["hp"] -= amount + _enemy_status_damage_bonus(enemy)
	if enemy["hp"] <= 0:
...
func _upgrade_cost(id: String) -> int:
	...
	match id:
		"damage", "ice_shatter", "fire_burst", "thunder_chain", "thunder_overload", "status_tip", "fire_coals", "thunder_wire":
			return base + 3
		"stun", "gem_xp", "magnet":
			return base - 1
		_:
...
func _upgrade_is_available(id: String) -> bool:
	...
	match id:
		...
		"prospector", "gem_vein", "rock_ledger":
...
func _tick_burning_enemy(enemy_i: int) -> bool:
	...
	enemy["hp"] -= 1 + _effective_burn_damage_bonus()
	if enemy["hp"] > 0:
...
func _damage_enemy(enemy_i: int, amount: int, kill_text: String, hit_text: String) -> bool:
	...
	enemy["attack_dir"] = Vector2i.ZERO
	enemy["hit_flash"] = ENEMY_HIT_FLASH
	enemy["hp"] -= amount + tunnel_bonus + _enemy_status_damage_bonus(enemy)
